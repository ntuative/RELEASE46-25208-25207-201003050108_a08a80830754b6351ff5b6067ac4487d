package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_419:
               ...
               {
                  case RoomSessionQueueEvent.const_913:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_920:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc5_.length > 1)
               {
                  if(_loc4_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_926) != -1)
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_926) + 1;
                     _loc7_ = true;
                  }
                  else
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1135) + 1;
                  }
               }
               else
               {
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_707:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_920);
               break;
            case RoomWidgetRoomQueueMessage.const_518:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_913);
               break;
            case RoomWidgetRoomQueueMessage.const_695:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_256,true);
               }
         }
         return null;
      }
